...
object TempIncreaseAlert {
  def main(args: Array[String]): Unit ={
    ...
    env
      ...
      .keyBy(_.id)
      .process(new TimeincreaseAlterFunction)
      .print()
  ...
  class TimeincreaseAlterFunction extends KeyedProcessFunction[String,SensorReading,String]{
    ...
    override def processElement(i: SensorReading, context: KeyedProcessFunction[String, SensorReading, String]#Context, collector: Collector[String]): Unit = {
      ...
      var preTemp  = lastTemp.value()
      // 将来的温度值更新到lastTemp状态变量, 使用update方法
      lastTemp.update(i.temperature)

      var curTs = timerTs.value()

      if(preTemp == 0.0 || i.temperature < preTemp){
        // 如果来的温度是第一条温度，或者来的温度小于最近一次温度
        // 删除报警定时器
        context.timerService().deleteProcessingTimeTimer(curTs)
        timerTs.clear()
      }else if(i.temperature > preTemp && curTs == 0L){
        // 来的温度大于最近一次温度，并且我们没有注册报警定时器，因为curTimerTs等于0L
        var ts = context.timerService().currentProcessingTime()+1000L
